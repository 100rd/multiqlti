...
const { ValidationError, RequiresHumanApproval, MissingRollbackProofError } = require('../../src/errors');
...
describe('SPEC-02 E2E Test Suite', () => {
  ...
  describe('Tier 2: Boundary & Corner Cases (22 Tests)', () => {
    ...
    describe('Feature 2.4: Proof Router - Boundary conditions for Proof validation', () => {
      ...
      test('T2_PR_PRF_04: Type E task with rollback_script set to empty string throws MissingRollbackProofError', () => {
        const task = { id: 't-45', type: 'E' };
        expect(() => router.submitCompletedTask(task, { rollback_script: '' })).toThrow(MissingRollbackProofError);
      });
