...
- Tests (20 new, all passing):
  - `tests/unit/skills/skill-md-service.test.ts` — valid variants, malformed
    YAML/missing frontmatter, oversized name/file/tags, non-semver version.
  - `tests/unit/skills/registry-sync.test.ts` — synced, skipped
    (compatible_tools), drift (no row created/updated), idempotent re-run
    (same row updated, not duplicated), fail-closed allowlist, missing lock
    file.
  - `tests/integration/skills-registry-sync-api.test.ts` — 401 unauthenticated
    (real `requireAuth`), 400 invalid body, 200 happy path (drift/skip/synced
    reported + synced row rejects PATCH/DELETE with 403 "git-sourced"), 400
    registryRoot outside the configured allowlist.
...
- `tsc --noEmit --pretty false`: clean (0 errors), checked after Phase 1 code
  and again after Phase 2 tests/fixtures.
- New test files: `vitest run tests/unit/skills/skill-md-service.test.ts
  tests/unit/skills/registry-sync.test.ts
  tests/integration/skills-registry-sync-api.test.ts` → 3 files passed, 20
  tests passed.
...
- Full unit/integration suite: `vitest run --reporter=dot` → **304 test
  files passed (304), 5169 tests passed | 5 skipped (5174), 0 failed**.
  Duration 57.67s.
